...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(`${VENDOR_API_URL}/v1/orders/${id}`, {
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: errorData.message || 'Error al obtener orden' },
    ...
    console.error('Error en GET /api/vendor/orders/[id]:', error);
    return NextResponse.json(
...
export async function PUT(
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      return NextResponse.json(
        { error: errorData.message || 'Error al actualizar orden' },
        { status: response.status }
      );
    }
    ...
    console.error('Error en PUT /api/vendor/orders/[id]:', error);
    return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      return NextResponse.json(
        { error: errorData.message || 'Error al eliminar orden' },
        { status: response.status }
      );
    }
    ...
    console.error('Error en DELETE /api/vendor/orders/[id]:', error);
    return NextResponse.json(

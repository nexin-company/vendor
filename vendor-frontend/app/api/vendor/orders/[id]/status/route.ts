...
export async function PUT(
  ...
  try {
    ...
    const response = await fetch(`${VENDOR_API_URL}/v1/orders/${id}/status`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'X-API-Key': VENDOR_API_KEY,
        // actor best-effort (para auditoría aguas abajo)
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: errorData.message || 'Error al actualizar status' },
    ...
    console.error('Error en PUT /api/vendor/orders/[id]/status:', error);
    return NextResponse.json(
      { error: error.message || 'Error al actualizar status' },
      { status: 500 }
    );
  }
}

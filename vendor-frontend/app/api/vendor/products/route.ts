...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = new URL(`${VENDOR_API_URL}/v1/products`);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${VENDOR_API_URL}/v1/products`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-API-Key': VENDOR_API_KEY,
    ...
    console.error('Error en POST /api/vendor/products:', error);
    return NextResponse.json(
